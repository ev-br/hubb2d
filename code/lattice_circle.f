...
!--- stores whether a site is 'active' or not
    logical, allocatable :: array_is_active(:)
...
!-------------------------------------------------------
    ...
    !print*, nneighb(:); stop
      ...
      Nsite_active = Nsite
      allocate(array_is_active(1:Nsite))
      array_is_active = .true.
      call mark_active()
...
!--- Mark active sites
!----------------------------------------------
    subroutine mark_active()
    implicit none
    real*8 :: center, radius
    integer :: site

      center = 1.0 + (N(1)-1.0) / 2.d0
      radius = N2(1)

      Nsite_active = 0

      array_is_active = .false.
      do site = 1, Nsite
        if(sum((x(:, site) - center)**2) < radius**2)then
            array_is_active(site) = .true.
            Nsite_active = Nsite_active + 1
        endif
      enddo

    end subroutine mark_active
...
!---------------------------------------------
    ...
    is_active = array_is_active(site)
